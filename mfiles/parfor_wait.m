classdef parfor_wait < handle
   ...
   methods (Access = private)
       ...
       function WaitbarUpdate(Obj)
            UsedTime_now = clock;%toc(Obj.StartTime);
%            EstimatedTimeNeeded = (UsedTime_now-Obj.UsedTime_1)/Obj.ReportInterval*(Obj.TotalMessage-Obj.NumMessage);
           
            progress = Obj.NumMessage/Obj.TotalMessage;
            elap = etime(UsedTime_now,Obj.UsedTime_1);
            sec_remain = elap*(1/progress-1);
            r_mes = datestr(sec_remain/86400,'HH:MM:SS');
                
                
%            class(EstimatedTimeNeeded)
%            EstimatedTimeNeeded
%            datestr(EstimatedTimeNeeded,'HH:MM:SS')
%            num2str(EstimatedTimeNeeded, '%.2f')
%             r_mes = datestr(EstimatedTimeNeeded/86400,'HH:MM:SS');
%            waitbar(Obj.NumMessage/Obj.TotalMessage, Obj.WaitbarHandle, [num2str(Obj.NumMessage/Obj.TotalMessage*100, '%.2f'), '%; ', num2str(UsedTime_now, '%.2f'), 's used and ', num2str(EstimatedTimeNeeded, '%.2f'), 's needed.']);
           waitbar(progress, Obj.WaitbarHandle,{['Frames processed: ',...
                    num2str(Obj.NumMessage) '/' num2str(Obj.TotalMessage)];...
...
%            Obj.UsedTime_1 = UsedTime_now;
       ...
       function FileUpdate(Obj)
           ...
           switch Obj.FileName
               case 'screen'
                    fprintf(' %.2f/%.2f frames regitered; remaining time %.2fs\n', Obj.NumMessage,Obj.TotalMessage,EstimatedTimeNeeded);
                     
%                    fprintf('%.2f%%; %.2fs used and %.2fs needed...\n', Obj.NumMessage/Obj.TotalMessage*100, UsedTime_now, EstimatedTimeNeeded);
               otherwise
                   fprintf(Obj.FileID,'%.2f/%.2f frames regitered; remaining time %.2fs\n', Obj.NumMessage,Obj.TotalMessage,EstimatedTimeNeeded);
